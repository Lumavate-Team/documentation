
.. _Resources:

=========
Resources
=========

________________________________________________________________________________________________________________________________________

.. include:: ../API.rst
______________________________________________________________________________________________________________________________________

.. _Code Samples:

Code Samples
------------

Code samples can be found at https://github.com/Lumavate-Community

.. _C# sample:

C#
^^

 Common libraries and scaffolding code for .NET Core with a lightweight shell for use with C#.

 C# property library: Coming Soon.
 
 C# signer library: Coming Soon.
 
 Starter container: https://github.com/Lumavate-Team/widget-base-csharp
 

.. _Go Sample:

GO
^^

 Common libraries and scaffolding code for use with Golang.

 Go property library: https://github.com/Lumavate-Team/lumavate-go-common
 
 Go signer library: Coming Soon.

 Starter container: https://github.com/Lumavate-Team/widget-base-go

.. _python sample:

Python
^^^^^^

 Common libraries and scaffolding code for use with Python.

 Python property library: https://github.com/LabelNexus/python-widget-properties

 Python signer library: https://github.com/Lumavate-Team/python-signer

 Starting containers:

 Simple API pass-through with example of every property type:https://github.com/Lumavate-Community/simple-api-service

 Starter with DB support: https://github.com/Lumavate-Community/starter-python-with-db

JSON
^^^^

 Example component set containers for use with JSON. 

 Starter component set container: Coming Soon. 

________________________________________________________________________________________________________________________________________

.. include:: ../GLOSSARY.rst
________________________________________________________________________________________________________________________________________
